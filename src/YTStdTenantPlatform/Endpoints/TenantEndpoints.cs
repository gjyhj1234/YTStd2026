    ...
    public static class TenantEndpoints
    {
        ...
        public static void Map(WebApplication app)
        {
            ...
            group.MapGet("/", async (HttpContext ctx, int? page, int? pageSize, string? keyword, string? status) =>
            {
                ...
                await WriteJsonAsync(ctx, ApiResult<PagedResult<TenantDto>>.Ok(result));
            ...
            group.MapGet("/{id:long}", async (HttpContext ctx, long id) =>
            {
                ...
                await WriteJsonAsync(ctx, ApiResult<TenantDto>.Ok(result));
            }).WithSummary("获取租户详情");

            group.MapPost("/", async (HttpContext ctx) =>
            {
                var user = GetCurrentUser(ctx);
                var req = await ctx.Request.ReadFromJsonAsync<CreateTenantRequest>();
            ...
            {
                var user = GetCurrentUser(ctx);
                var req = await ctx.Request.ReadFromJsonAsync<UpdateTenantRequest>();
                if (req == null) { await WriteJsonAsync(ctx, ApiResult.Fail("请求体无效"), 400); return; }
            ...
            {
                var user = GetCurrentUser(ctx);
                var req = await ctx.Request.ReadFromJsonAsync<TenantStatusChangeRequest>();
                if (req == null) { await WriteJsonAsync(ctx, ApiResult.Fail("请求体无效"), 400); return; }
            ...
            group.MapGet("/{id:long}/lifecycle-events", async (HttpContext ctx, long id, int? page, int? pageSize) =>
            {
                ...
                await WriteJsonAsync(ctx, ApiResult<PagedResult<TenantLifecycleEventDto>>.Ok(result));
