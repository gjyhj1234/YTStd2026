    ...
    public static class PlatformUserEndpoints
    {
        ...
        public static void Map(WebApplication app)
        {
            ...
            group.MapGet("/", async (HttpContext ctx, int? page, int? pageSize, string? keyword, string? status) =>
            {
                ...
                await WriteJsonAsync(ctx, ApiResult<PagedResult<PlatformUserDto>>.Ok(result));
            ...
            group.MapGet("/{id:long}", async (HttpContext ctx, long id) =>
            {
                ...
                await WriteJsonAsync(ctx, ApiResult<PlatformUserDto>.Ok(result));
            }).WithSummary("获取平台用户详情");

            group.MapPost("/", async (HttpContext ctx) =>
            {
                var user = GetCurrentUser(ctx);
                var req = await ctx.Request.ReadFromJsonAsync<CreatePlatformUserRequest>();
            ...
            {
                var user = GetCurrentUser(ctx);
                var req = await ctx.Request.ReadFromJsonAsync<UpdatePlatformUserRequest>();
                if (req == null) { await WriteJsonAsync(ctx, ApiResult.Fail("请求体无效"), 400); return; }
