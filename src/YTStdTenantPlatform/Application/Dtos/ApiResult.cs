...
namespace YTStdTenantPlatform.Application.Dtos
{
    ...
    public sealed class ApiResult
    {
        ...
        /// <summary>消息（保留字段，默认为空）</summary>
        public string Message { get; set; } = "";

        /// <summary>成功结果</summary>
        public static ApiResult Ok()
            => new ApiResult { Code = 0, Message = "" };

        /// <summary>失败结果</summary>
        public static ApiResult Fail(int code)
            => new ApiResult { Code = code, Message = "" };
    }
    ...
    public sealed class ApiResult<T>
    {
        ...
        public static ApiResult<T> Ok(T data)
            => new ApiResult<T> { Code = 0, Message = "", Data = data };

        /// <summary>失败结果</summary>
        public static ApiResult<T> Fail(int code)
            => new ApiResult<T> { Code = code, Message = "" };
    }
