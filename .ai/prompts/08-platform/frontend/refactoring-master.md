...
## 八、质量检查清单（每模块完成后 Agent 自检）

1. **grep -rn "硬编码中文检测"**：搜索 `.vue` 文件中未使用 `t()` 包裹的中文字符串
2. **grep -rn "confirm\|notify"**：确认所有操作有反馈
3. **grep -rn "validationRules"**：确认表单有验证规则
4. **grep -rn "perm.has"**：确认操作按钮有权限控制
5. **文件计数**：确认每个 `.vue` 文件有 5 个对应的 `.vue.{locale}.json` 文件
