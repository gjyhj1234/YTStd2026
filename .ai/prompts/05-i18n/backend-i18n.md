...
为后端模块接入国际化支持，确保所有用户可见消息使用整形常量 ErrorCodes，后端零文本输出，并通过 YTStdI18n.Generator 自动生成前端语言包。
...
- 相关服务代码中硬编码中文替换为 `ApiResult.Fail(ErrorCodes.XXX)` 单参数调用
...
2. 检查是否仅使用 `ErrorCodes.XXX` 单个参数（不传 message 参数）
3. 扫描所有硬编码中文字符串，替换为 `ApiResult.Fail(ErrorCodes.XXX)` 模式
4. 在 ErrorCodes.cs 中添加缺失的错误码常量（`const int`）
...
9. 验证 generated 中的 Code 与后端常量一一对应
10. 编译通过且无警告
...
// 前端通过 t(templateCode.toString()) 翻译
...
- 生成前端 `locales/generated/{locale}.json`

### 增量生成策略

- 遍历后端定义的所有整形 Code
- 检查前端对应语言文件中该 Code 是否存在
- **已存在 → 不修改**（保留人工翻译校正）
- **不存在 → 新增**（使用默认翻译或空字符串）
...
- ErrorCodes 常量为 `const int` 类型
- 后端不保留任何用户可见的文本字符串
- 后端不存在 Messages 类（已移除）
...
- 禁止 `ApiResult.Fail()` 传入 message 参数（仅允许单参数 ErrorCode）
- 禁止硬编码中文字符串作为 API 错误消息
- 禁止使用字符串字面量作为 i18n key
...
- [ ] 所有 `ApiResult.Fail()` 仅传 `ErrorCodes.XXX`（单参数）
- [ ] ErrorCodes 常量为 `const int` 类型
- [ ] 后端不存在 Messages 类
