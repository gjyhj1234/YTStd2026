...
### 当前所处阶段
- 阶段 G（前端全面重构）— G1-FIX / G2-FIX 全部完成 **✅ 完成**
...
### 风险与待确认
1. ~~Issue #3 备注字段需后端同步修改~~ ✅ 已完成（PlatformUserRepDTO + MapToDto）
2. Issue #5 用户角色分配：当前从角色侧绑定成员，用户侧无法预加载已有角色（后端缺接口）
3. ~~Issue #8 安全设置~~ ✅ 已实现密码修改功能，IP白名单/MFA标注"开发中"
