...
> 详细任务说明、子任务拆分与验收标准见独立文件：
> - `.ai/tasks/task-platform-frontend.md` — 前端重构子任务定义
> - `.ai/prompts/08-platform/frontend/refactoring-master.md` — 前端重构总纲提示词

### 背景

阶段 E（前端初版）和 F（国际化初版）虽已标记完成，但经全面审查发现前端代码仅为 Demo 级别，存在大量严重缺陷，需进行系统性重构。

### 发现的主要问题类别

1. **i18n 致命缺陷**：组件级语言文件完全缺失（37 个视图无一创建）、FunctionDescriptionCard/OperationGuideDrawer 内容硬编码中文、枚举翻译不完整、ms-MY/zh-TW 翻译缺失、ja-JP 语言完全缺失
2. **表单验证完全缺失**：所有表单无 validationRules、无唯一性实时检查、无提交前 validate()
3. **操作反馈缺失**：操作成功无 Toast 提示、删除无确认弹窗、错误提示不可见、catch 静默吞错
4. **业务功能残缺**：远程分页未实现、详情页缺失、批量操作缺失、多个 API 路径与后端不匹配
5. **组件规范违规**：大量使用原生 HTML 替代 DevExtreme 组件、自定义 CSS 硬编码颜色
6. **权限控制不完整**：部分操作按钮缺少权限控制、无 403 页面
...
| G4 — 租户配置模块 | ⬜ 待执行 | 系统配置/功能开关/参数 |
| G5 — SaaS 套餐订阅 | ⬜ 待执行 | 套餐/版本/订阅/试用 |
| G6 — 计费支付模块 | ⬜ 待执行 | 发票/支付/退款 |
| G7 — API 集成模块 | ⬜ 待执行 | API 密钥/Webhook/投递日志 |
| G8 — 运营日志模块 | ⬜ 待执行 | 运营统计/操作日志/审计日志/系统日志 |
| G9 — 通知存储模块 | ⬜ 待执行 | 通知模板/通知/存储策略/文件 |
| G10 — 基础设施系统设置 | ⬜ 待执行 | 基础设施/限流/菜单/字典 |
