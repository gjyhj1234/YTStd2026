...
1. **一个子任务文件 = 一个模块 Epic；一次 Agent 会话 = 一个 slice**
...
1. `.ai/tasks/platform-frontend/*.md` 中的模块文件只定义 **Epic**，不再等价于“一次性做完整个模块”。
2. 实际执行必须先拆为 slice，推荐拆分为：
...
1. GitHub Copilot coding agent 只应被指派到单个 slice。
2. 每轮结束必须更新任务快照和 session summary，而不是只更新聊天输出。
...
## 本轮目标
完成子任务 {编号} 的当前 slice，而不是一次性做完整个模块
```
