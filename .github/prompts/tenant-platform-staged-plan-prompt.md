...
10. **主键 ID 必须显式生成**：所有 `InsertAsync` 之前必须调用 `entity.Id = await DB.GetNextLongIdAsync();`，禁止依赖数据库自动分配 ID。需引入 `using YTStdAdo;`。
11. **唯一性校验**：所有包含唯一索引的实体，创建和更新时必须进行应用层唯一性校验，并提供 `check-exists` 接口供前端调用。
12. **i18n 消息键**：所有 `ApiResult.Fail(...)` 的 `message` 必须使用 `Messages.XXX` 常量（i18n 键），**禁止硬编码中文字符串**。
13. **Logger.Debug 必须使用 `Func<string>` 委托重载**，不使用直接字符串参数或字符串插值。
