...
1. **YTStdEntity**（运行时库）：提供实体基础类型（`DbNullable<T>`、`DBNULL`、`YTFieldMeta`）和实体特性（`EntityAttribute`、`ColumnAttribute`、`IndexAttribute`），以及增量备份与租户分离运行时逻辑。
2. **YTStdEntity.Generator**（Roslyn Source Generator）：在编译期扫描标注了 `[Entity]` 的实体类，自动生成三类代码：
...
### 9.2 `IncrementalBackupOptions` 配置
...
### 10.2 `TenantSeparationOptions` 配置
